...
/**
 * `prisma/schema/resources.prisma`: every model the descriptors imply, in a stable order.
 *
 * Only the models. Prisma 7 reads a folder of schema files, which is what keeps this one
 * safe to overwrite: the generator block, the datasource and the tables Flare's own
 * features need — users, sessions, the audit log — live in `base.prisma` beside it and
 * are yours to edit.
 */
